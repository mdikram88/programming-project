<template>
  <div class="bg-gradient-primary">
    <div class="container">
      <!-- Outer Row -->
      <div class="row justify-content-center">
        <div class="col-xl-10 col-lg-12 col-md-9">
          <div class="card o-hidden border-0 shadow-lg my-5">
            <div class="card-body p-0">
              <!-- Nested Row within Card Body -->
              <div class="row text-center">
                <div class="col-lg-6 d-none d-lg-block">
                  <img
                    src="../assets/forget_password.jpg"
                    alt="Forget Password"
                    width="475"
                    height="650"
                    style="background-size: cover; background-position: center"
                  />
                </div>
                <div class="col-lg-6">
                  <div class="p-5">
                    <div class="text-center">
                      <h4 class="h4 text-gray-900 mb-2">
                        Forgot Your Password?
                      </h4>
                      <p class="mb-4">
                        We get it, stuff happens. Just enter your email address
                        below and we'll send you a link to reset your password!
                      </p>
                    </div>
                    <!----Message---->
                    <div class="row g-1">
                      <div
                        v-if="msg"
                        class="col-md-12 alert alert-success"
                        role="alert"
                      >
                        <br />
                        <h5 class="alert-heading">🎉 {{ msg }} 🎉</h5>
                      </div>
                    </div>
                    <form class="user" @submit="validate" novalidate>
                      <div v-if="errors.length" class="col-md-12">
                        <br />
                        <ul
                          v-for="error in errors"
                          :key="error"
                          class="list-group list-group-flush"
                        >
                          <li class="list-group-item list-group-item-danger">
                            <span style="font-weight: bold; color: red">
                              {{ error }}
                            </span>
                          </li>
                        </ul>
                        <hr />
                      </div>
                      <div class="form-group">
                        <input
                          type="text"
                          class="form-control form-control-user"
                          v-model="email"
                          id="inputEmail"
                          aria-describedby="EmailHelp"
                          placeholder="Email"
                        />
                      </div>
                      <div class="form-group">
                        <input
                          type="text"
                          class="form-control form-control-user"
                          id="inputBackup"
                          v-model="backup"
                          placeholder="Backup Code"
                        />
                        <small class="text-muted"
                          >Enter the Backup Code you gave on account
                          creation</small
                        >
                      </div>
                      <div class="form-group row">
                        <div class="col-sm-6 mb-3 mb-sm-0">
                          <input
                            :type="seen ? 'text' : 'password'"
                            v-model="password"
                            class="form-control form-control-user"
                            id="inputPassword"
                            placeholder="New Password"
                          />
                        </div>
                        <div class="col-sm-6">
                          <input
                            :type="seen ? 'text' : 'password'"
                            class="form-control form-control-user"
                            id="inputPassword2"
                            v-model="password2"
                            placeholder="Confirm Password"
                          />
                        </div>
                        <small class="text-muted"
                          >Password Should be at least 8 characters</small
                        >
                      </div>
                      <div class="form-group">
                        <div
                          class="form-check col-md-6"
                          @click="toggleSeen"
                          style="padding-left: 30px"
                        >
                          <input
                            type="checkbox"
                            class="form-check-input"
                            id="Seen"
                          />
                          <label id="check" class="form-check-label" for="Seen"
                            >Show Password</label
                          >
                        </div>
                      </div>
                      <button
                        type="submit"
                        class="btn btn-primary btn-user btn-block"
                      >
                        Reset Password
                      </button>
                      <hr />
                    </form>
                    <div class="text-center" style="font-size: large">
                      <router-link class="small" :to="{ name: 'CreateAccount' }"
                        >No account? Register Now</router-link
                      >
                    </div>
                    <div class="text-center" style="font-size: large">
                      <router-link class="small" :to="{ name: 'Login' }"
                        >Already have password ? Login</router-link
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "ResetView",
  data() {
    return {
      email: "",
      backup: "",
      seen: false,
      pass: true,
      password: "",
      password2: "",
      errors: [],
      msg: "",
    };
  },
  methods: {
    toggleSeen: function () {
      this.seen = !this.seen;
    },
    validate: function (e) {
      this.errors = [];
      if (!this.email.trim()) {
        this.errors.push("Please Fill Email");
      }
      if (!this.backup.trim()) {
        this.errors.push("Please Fill Backup Code");
      } else if (this.backup.trim().length < 6) {
        this.errors.push("Backup Code is at least 6 characters");
      }
      if (!this.password.trim()) {
        this.errors.push("Please Fill Password");
      } else if (this.password.trim().length < 8) {
        this.errors.push("Password Should be at least 8 characters");
      }
      if (!this.password2.trim()) {
        this.errors.push("Please Fill Confirm Password");
      }
      if (this.password.trim() !== this.password2.trim()) {
        this.errors.push("Password and Confirm password do not match");
      }
      if (!this.errors.length) {
        this.resetMethod();
      }
      e.preventDefault();
    },
    resetMethod: function () {
      this.pass = true;
      let user = {
        email: this.email,
        backup: this.backup,
        password: this.password,
      };
      fetch("http://127.0.0.1:8000/reset_password", {
        method: "post",
        omit: "no-cors",
        credentials: "omit",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(user),
      })
        .then((response) => {
          if (response.status !== 200) {
            this.pass = false;
          }
          return response.json();
        })
        .then((data) => {
          if (this.pass) {
            this.msg = data["message"];
            setTimeout(this.divert, 3000);
          } else {
            console.log(data);
            this.errors = [];
            if (data["message"]) {
              this.errors.push(data["message"]);
            }
          }
        })
        .catch((error) => {
          console.log(error);
        });
    },
    divert: function () {
      this.$router.push({ name: "Login" });
    },
  },
};
</script>

<style scoped>
h1 {
  font-weight: bold;
  font-size: xxx-large;
}
</style>
